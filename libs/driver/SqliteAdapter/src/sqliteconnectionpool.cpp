#include <SqliteAdapter/sqliteconnectionpool.hpp>
...
std::chrono::seconds connection_pool::idle_timeout = std::chrono::seconds(300);

std::shared_ptr<connection_pool> connection_pool::instance()
{
    static auto pool = std::make_shared<connection_pool>(connection_settings, start_pool_size, max_pool_size, wait_time, idle_timeout);
    return pool;
}

connection_pool::connection_pool(database_connection_settings settings,
    const size_t start_pool_size,
    const size_t max_pool_size,
    std::chrono::seconds wait_time,
    std::chrono::seconds idle_timeout)
    : IConnectionPool(std::move(settings), start_pool_size, max_pool_size, wait_time, idle_timeout)
{
}
